...
from backend.history.cosmosdbservice import CosmosConversationClientForUseCase
...
# Initialize Cosmos DB client for usecase
AZURE_USECASE_COSMOSDB_ACCOUNT = os.environ['AZURE_USECASE_COSMOSDB_ACCOUNT']
AZURE_USECASE_COSMOSDB_DATABASE = os.environ['AZURE_USECASE_COSMOSDB_DATABASE']
AZURE_USECASE_COSMOSDB_QUERY_CONTAINER = os.environ['AZURE_USECASE_COSMOSDB_QUERY_CONTAINER']
AZURE_USECASE_COSMOSDB_ACCOUNT_KEY = os.environ['AZURE_USECASE_COSMOSDB_ACCOUNT_KEY']
...
cosmos_usecase_conversation_client = None
if AZURE_USECASE_COSMOSDB_DATABASE and AZURE_USECASE_COSMOSDB_ACCOUNT and AZURE_USECASE_COSMOSDB_QUERY_CONTAINER:
    try :
        cosmos_endpoint = f'https://{AZURE_USECASE_COSMOSDB_ACCOUNT}.documents.azure.com:443/'
        print(cosmos_endpoint)
        credential = AZURE_USECASE_COSMOSDB_ACCOUNT_KEY

        cosmos_usecase_conversation_client = CosmosConversationClientForUseCase(
            cosmosdb_endpoint=cosmos_endpoint, 
            credential=credential, 
            database_name=AZURE_USECASE_COSMOSDB_DATABASE,
            container_name=AZURE_USECASE_COSMOSDB_QUERY_CONTAINER
        )
    except Exception as e:
        logging.exception("Exception in Usecase CosmosDB initialization", e)
        cosmos_usecase_conversation_client = None
...
# Get Usecase List
@app.route("/useCase/name_list", methods=["GET"])
@cross_origin()
def list_useCase():
    try:
        result = cosmos_usecase_conversation_client.get_usecase_names_query()
        return jsonify({"response": result})
    except Exception as e:
        logging.exception("Error in getting use case list", e)
